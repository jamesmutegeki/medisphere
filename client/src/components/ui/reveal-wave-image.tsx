...
const fragmentShader = `
  ...
  float bayer4x4(vec2 pos) {
    ...
    for (int i=0;i<16;i++) { if (i==index) return pattern[i]/16.0; }
    return 0.0;
  }
  ...
  void main() {
    ...
    if (uMouseActive > 0.01) {
      ...
      float ripple = sin(dist*rippleFreq-time*uWaveSpeed)*uWaveAmplitude*0.05*mouseInfluence*uMouseActive;
    ...
    float quantized = adjusted<0.33 ? 0.0 : (adjusted<0.66 ? 0.5 : 1.0);
...
function ImagePlane({
  src, aspectRatio, revealRadius, revealSoftness, pixelSize,
  waveSpeed, waveFrequency, waveAmplitude, mouseRadius, isMouseInCanvas,
}: ImagePlaneProps) {
  ...
  return (
    ...
    >
      {aspectRatio !== null && (
        ...
        >
          <ImagePlane
            src={src} aspectRatio={aspectRatio}
            revealRadius={revealRadius} revealSoftness={revealSoftness}
            pixelSize={pixelSize} waveSpeed={waveSpeed}
            waveFrequency={waveFrequency} waveAmplitude={waveAmplitude}
            mouseRadius={mouseRadius} isMouseInCanvas={isMouseInCanvas}
          />
        </Canvas>
      )}
    </div>
