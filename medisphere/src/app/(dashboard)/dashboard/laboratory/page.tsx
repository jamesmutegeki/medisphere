...
import { FlaskConical, Search, Activity, Loader2, AlertCircle, Download, Eye } from 'lucide-react';
...
export default function LaboratoryPage() {
    ...
    async function fetchData() {
      try {
        ...
        setLabResults(items.map((r: any) => ({
          ...
          category: r.testCategory || r.category || '',
          result: r.result || '',
          range: r.normalRange || r.range || '',
          status: r.status || 'PENDING',
          date: r.date || r.createdAt || r.performedAt || '',
          unit: r.unit || '',
        })));
      } catch (err) {
  ...
  const filteredResults = labResults.filter((r) =>
    isPatient ? true : r.patient.toLowerCase().includes(searchQuery.toLowerCase())
  );

  const downloadResult = (result: any) => {
    const content = [
      `Lab Result: ${result.test}`,
      ...
      '',
      '--- Generated by MediSphere ---',
    ].join('\n');
    const blob = new Blob([content], { type: 'text/plain' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `lab-result-${result.id}.txt`;
    a.click();
        ...
        <CardContent>
              ...
              <tbody>
                {filteredResults.map((result, index) => (
                  ...
                  >
                    ...
                    <td className="py-3 px-4 text-gray-500">{new Date(result.date).toLocaleDateString()}</td>
                    <td className="py-3 px-4">
          ...
          {filteredResults.length === 0 && (
            <div className="text-center py-12 text-gray-400">
              ...
              <p>{isPatient ? 'No lab results found' : `No lab results found for "${searchQuery}"`}</p>
            </div>
          )}
        </CardContent>
      </Card>
